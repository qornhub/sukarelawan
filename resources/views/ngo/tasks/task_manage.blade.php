...
<div class="card mt-4 participant-section {{ $disabled ? 'is-disabled' : '' }}" id="section-manage-tasks"
    style="display:none" data-disabled="{{ $disabled ? '1' : '0' }}" aria-disabled="{{ $disabled ? 'true' : 'false' }}">

    <div class="card-body">
        <h3 class="mb-3">Manage Tasks</h3>
        <div id="manage-task-flash"
     class="task-flash-area mb-2"
     aria-live="polite"
     aria-atomic="true"></div>
        ...
        <div class="manage-task-card-list">
            ...
            @forelse ($tasks as $task)
                <div class="manage-task-card" data-task-id="{{ (string) $task->task_id }}"
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        ...
                        {{-- ACTION --}}
                        @if ($disabled)
                            <button type="button" class="btn btn-sm btn-outline-success assign-btn is-disabled"
                                data-task-id="{{ (string) $task->task_id }}" aria-disabled="true" tabindex="-1"
                            ...
                            <span class="badge bg-primary me-1 assigned-badge"
                                ...
                                @if ($disabled)
                                    <button type="button"
                                        ...
                                        data-user-id="{{ (string) $a->user_id }}" aria-disabled="true" tabindex="-1"
                                        title="Unassign disabled — event ended">
                                    </button>
                                @else
                                    <button type="button" class="btn-close btn-close-white btn-sm ms-1 unassign-btn"
                                        aria-label="Remove" data-task-id="{{ (string) $task->task_id }}"
                                        data-user-id="{{ (string) $a->user_id }}">
                                    </button>
                                @endif
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                @if ($disabled)
                    <button type="button" class="btn btn-primary" id="assign-submit" disabled aria-disabled="true"
                        tabindex="-1">
                        Assign
                    </button>
                @else
    ...
    <script>
        ...
        `;
            ...
            document.addEventListener('click', async function(e) {
                ...
                if (!btn || !manageSection.contains(btn))
                    return; // ⭐ only respond within manage section
            ...
            if (assignForm) {
                assignForm.addEventListener('submit', async function(e) {
                    ...
                    try {
                        ...
                        if (!resp.ok) {
                            flash('Failed to assign participants', 'error');
                            return;
                        }
                        ...
                        assignModal.hide();
                        flash('Participants assigned successfully', 'success');
...
document.addEventListener('click', async function (e) {
    ...
    if (!btn || !manageSection.contains(btn)) return;

    e.preventDefault();

    if (manageSection?.dataset?.disabled === '1') {
        flash('Task management is disabled — this event has ended.', 'error');
        return;
    }
    ...
    if (!taskId || !userId) {
        flash('Internal error — missing task or user ID.', 'error');
        return;
    }
    ...
    try {
        ...
        if (!resp.ok) {
            flash('Failed to unassign participant', 'error');
            return;
        }
        ...
        // Update data-assigned CSV so modal stays accurate
        const row = getRowByTaskId(taskId);
        if (row) {
            const csv = (row.getAttribute('data-assigned') || '')
                .split(',')
                .map(s => s.trim())
                .filter(Boolean)
                .filter(id => id !== String(userId));

            row.setAttribute('data-assigned', csv.join(','));
        }

        flash('Participant unassigned successfully', 'success');

        // If no badges left, show —
        const container = getRowByTaskId(taskId)?.querySelector('.assigned-users');
        if (container && !container.querySelector('.assigned-badge')) {
            container.innerHTML = '<span class="text-muted">—</span>';
        }

    } catch (err) {
        console.error('[unassign] network error', err);
        flash('Failed to unassign participant', 'error');
    }
});
            ...
            function flash(text, type = 'success', duration = 2400) {
                const container =
                    document.querySelector('#manage-task-flash') ||
                    document.querySelector('#task-list-flash') ||
                    document.body;
            ...
            window.addConfirmedParticipantForTasks = function(vol) {
                ...
                const placeholder = participants.querySelector(
                    '.no-confirmed-placeholder, .text-muted.small.px-3');
