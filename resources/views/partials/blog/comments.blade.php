    ...
    <div class="comments-list">
        @forelse ($comments as $comment)
            @php
                ...
                $isAdmin =
                    auth()->check() &&
                    ((isset(auth()->user()->role->roleName) &&
                        strtolower(auth()->user()->role->roleName) === 'admin') ||
                ...
                $avatarUrl = $filename ? asset('images/profiles/' . $filename) : asset('assets/default-profile.png');
                ...
                if ($user->volunteerProfile) {
                    ...
                    $profileRoute = Route::has('ngo.profile.show') ? route('ngo.profile.show', $user->id) : '#';
                } else {
                    $profileRoute = '#';
                }
                ...
                <div class="d-flex">
                    ...
                    <div class="me-3">
                        <a href="{{ $profileRoute }}" title="{{ $displayName }}">
                            <img src="{{ $avatarUrl }}" class="avatar-circle" alt="{{ $displayName }}"
                    ...
                    <div class="flex-grow-1">
                        ...
                        @if ($isOwner)
                            <form id="comment-edit-{{ $comment->blogComment_id }}"
                                ...
                                method="POST" class="edit-comment-form" style="display:none; margin-top:.5rem;">
                                @csrf
                                @method('PUT')
