    ...
    <div class="admin-container">
        <div class="page-header">
            <div class="header-top">
                <h1 class="page-title">Badge Categories</h1>
                <a href="{{ route('admin.badge_categories.create') }}" class="btn btn-primary">
                    <i class="fas fa-plus me-2"></i>Add Badge Category
                </a>
            </div>
        </div>
        ...
        @if($categories->isEmpty())
            <div class="empty-state">
                ...
                <p class="text-muted">Get started by adding your first badge category.</p>
                <a href="{{ route('admin.badge_categories.create') }}" class="btn btn-primary mt-2">
                ...
                @foreach($categories as $index => $category)
                    <div class="category-card">
                        <div class="category-content">
                            <h3 class="category-name">{{ $category->badgeCategoryName }}</h3>
                            <p class="category-date">Created: {{ $category->created_at ? $category->created_at->format('Y-m-d') : '-' }}</p>
                            <div class="category-actions">
                                <a href="{{ route('admin.badge_categories.edit', $category->badgeCategory_id) }}" class="btn-action btn-edit">
                                    <i class="fas fa-edit me-1"></i> Edit
                                </a>
                                <form action="{{ route('admin.badge_categories.destroy', $category->badgeCategory_id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this category?');">
                                    @csrf
                                    @method('DELETE')
