    ...
    <div class="admin-container">
        ...
        @if($skills->isEmpty())
            <div class="empty-state">
                ...
                <a href="{{ route('admin.skill.create') }}" class="btn btn-primary mt-2">
                    <i class="fas fa-plus me-2"></i>Create New Skill
                </a>
            </div>
        @else
                ...
                @foreach($skills as $skill)
                    <div class="skill-card">
                        <div class="skill-content">
                            <p class="skill-id">ID: {{ $skill->skill_id }}</p>
                            <h3 class="skill-name">{{ $skill->skillName }}</h3>
                            <p class="skill-date">Created: {{ $skill->created_at ? $skill->created_at->format('Y-m-d') : '-' }}</p>
                            <div class="skill-actions">
                                <a href="{{ route('admin.skill.edit', $skill->skill_id) }}" class="btn-action btn-edit">
                                    <i class="fas fa-edit me-1"></i> Edit
                                </a>
                                <form action="{{ route('admin.skill.destroy', $skill->skill_id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
